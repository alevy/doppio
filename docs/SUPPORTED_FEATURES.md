...
| `C N1 X = N2 Y` commodity-conversion directive | + | (#248 stage 2, #266, #274). X (LHS) is the canonical commodity; Y (RHS) postings divide by `N2/N1` to yield X-equivalent amounts (principled math: `N1 X = N2 Y` → `1 Y = (N1/N2) X`). Both N1 and N2 may be arbitrary positive values; ledger-cli only honours the `N1 = 1` form, but doppio accepts and applies the formula for any N1. Transitive chains are resolved to the chain root via an eager fixpoint pass: `C 1s = 100c` + `C 1G = 100s` converts `c`-postings all the way to `G` (divisor 10000). Cycles (`C 1X = 1Y; C 1Y = 1X`) surface as `ResolutionError::CommodityConversionCycle`. A zero LHS amount (`C 0 X = ...`) produces `ResolutionError::InvalidCommodityConversion`. |
