...
use crate::{ast::{self, AmountDetails, ValueExpr}, resolution};
...
impl RunningState {
    fn minify(mut self) -> RunningState {
        ...
        self.account_balances.retain(|_, ab| !ab.commodity.is_empty());
        self
    }
}
        ...
        for entry in value.entries {
            ...
            match entry.data {
                resolution::Entry::Transaction(mut transaction) => {
                    ...
                    let payee = transaction.metadata.remove("payee").unwrap_or_else(|| transaction.description.clone());
                    let mut null_postings = vec![];
                    let mut resolved_postings = vec![];
                    for mut posting in transaction.postings {
                        if let Some(amount) = posting.amount {
                            let account_name = entry_context.account_aliases.get(&posting.account).cloned().unwrap_or(posting.account);
                            let account_balance = state.account_balances.entry(account_name.clone()).or_default();
                            let (value, commodity, lot_pricing) = match amount {
                                ...
                                AmountDetails::BalanceAssignment(assignment) => {
                                    let (newsum, commodity) = evaluator::eval_and_normalize_amount(assignment, &entry_context)?;
                                    let value = newsum - account_balance.commodity.get(&commodity).unwrap_or(&Decimal::ZERO);
                    ...
                    if let Some(mut posting) = null_postings.pop() {
                        ...
                        let amount = Amount(transaction_state.0.iter().map(|(c, v)| (c.clone(), -v)).collect());
                        ...
                        if transaction_state.0.values().any(|value| !value.is_zero()) {
                            return Err(ElaborationError::TransactionDoesNotBalance(transaction_state));
                        }
                    }
                    ...
                    for posting in resolved_postings.iter() {
                        let balances = state.account_balances.entry(posting.account.clone()).or_default();
...
mod evaluator {
    ...
    pub fn eval_and_normalize_amount(val: ast::ValueExpr, eval_context: &resolution::Context) -> Result<(Decimal, String), ElaborationError> {
        match eval(val)? {
            ast::ValueExpr::Amount { value, commodity } => {
                let commodity = if let Some(commodity) = commodity {
                    eval_context.commodity_aliases.get(&commodity).unwrap_or(&commodity).clone()
                } else {
                    eval_context.default_commodity.clone().ok_or(ElaborationError::AmountWithNoCommodity)?
                };
                Ok((value, commodity))
            },
        ...
        match val {
            ...
            ast::ValueExpr::Binary { lhs, rhs, op } => {
                match (eval(*lhs)?, eval(*rhs)?) {
                    ...
                    (ast::ValueExpr::Commodity(commodity), ast::ValueExpr::Amount { value, commodity: None }) |
                    (ast::ValueExpr::Amount { value, commodity: None }, ast::ValueExpr::Commodity(commodity)) => {
                        match op {
                            ast::Op::Sub => Ok(ast::ValueExpr::Amount {
                                value: -value,
            ...
            ast::ValueExpr::Function { name, args } => {
                match (name.as_str(), args.as_slice()) {
                    ...
                    ("account", [_account]) => {
                        Ok(ast::ValueExpr::Object(BTreeMap::from([("total".into(), ast::ValueExpr::Amount {
                            value: Decimal::ZERO,
                            commodity: Some("$".into())
                        })])))
                    },
            ...
            ast::ValueExpr::Typed { expr, commodity: new_commodity } => {
                match eval(*expr)? {
                    ast::ValueExpr::Amount { value, commodity } if commodity.is_none() || commodity.as_ref() == Some(&new_commodity) => {
                        Ok(ast::ValueExpr::Amount { value, commodity: Some(new_commodity) })
                    },
                    a => Err(EvaluationError::TypedCommodityToIncompatibleAmount((new_commodity, a))),
                }
            },
            ast::ValueExpr::Access { expr, field } => {
                match eval(*expr)? {
                    ast::ValueExpr::Object(map) => {
                        map.get(&field).cloned().ok_or(EvaluationError::NoSuchField(field))
                    },
